#!/usr/bin/env python3
"""
scripts/setup_envs.py â€” SaaS Blueprint V1 (project-level)
==========================================================
Creates venvs and installs dependencies. Reads requirements/install.txt.

Entry types (detected by filename):
  services/backend/requirements/install.txt  â†’ pip: .venv at services/backend/
  requirements/common.txt                    â†’ pip: .venv at project root
  services/frontend/package.json             â†’ npm install (always idempotent)
  services/backend                           â†’ delegates to setup_venv.py (legacy)
...
def read_entries() -> list[dict]:
    """Parse install.txt â†’ [{name, path, kind}, ...]."""
...
def setup_requirements_file(entry: dict, force: bool) -> bool:
    ...
    if force and venv_dir.exists():
        ...
        _rmtree(venv_dir)
        if venv_dir.exists():
            print("   âš ï¸  Venv parcialmente removido â€” arquivos em uso por outro processo.")
            print("      Feche qualquer terminal usando este venv e tente novamente.")
            print(close)
            return False

    # Broken venv: directory exists but pyvenv.cfg is missing â€” recreate
    if venv_dir.exists() and not (venv_dir / "pyvenv.cfg").exists():
        print("   âš ï¸  Venv corrompido (pyvenv.cfg ausente) â€” recriando...")
        _rmtree(venv_dir)
